...
function page(clientId: string, session: string, configured: boolean) {
  ...
  const action = configured
    ...
    : `<div class="notice">PayPal live credentials still need to be connected before a real charge can run.</div>`;
  return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8" />
...
${paypalScript}
<style>
:root{color-scheme:dark}*{box-sizing:border-box}body{margin:0;background:#09070b;color:#fff;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;min-height:100vh}.wrap{max-width:520px;margin:0 auto;padding:28px 20px 60px}.brand{font-size:12px;letter-spacing:2px;font-weight:900;color:#c699ff}.hero{margin-top:10px;padding:24px;border:1px solid #68408a;border-radius:26px;background:linear-gradient(145deg,#4b236d,#1d1029 55%,#110b17)}h1{margin:0;font-size:30px}.muted{color:#b9adbf;line-height:1.45}.pack{margin:18px 0;padding:18px;border-radius:20px;border:1px solid #43304d;background:#17111d}.row{display:flex;align-items:center;justify-content:space-between;gap:12px}.coins{font-size:28px;font-weight:900}.price{font-size:22px;font-weight:900}.pill{display:inline-block;margin-top:8px;padding:5px 9px;border-radius:999px;background:#332241;color:#d9bbff;font-size:11px;font-weight:800}.notice{padding:16px;border-radius:16px;background:#2a1c14;border:1px solid #6e4a2f;color:#ffd8ae}.success{padding:18px;border-radius:18px;background:#13271a;border:1px solid #2f6d43;color:#d8ffe3;display:none}.error{margin-top:14px;color:#ffabb8;font-size:13px;min-height:18px}.fine{font-size:11px;color:#887d90;line-height:1.5;margin-top:20px}</style>
</head>
<body><main class="wrap">
<div class="brand">FAMEVERSE</div>
<div class="hero"><h1>Recharge</h1><p class="muted">Owner QA checkout. This proves the real payment → verified capture → Fame Coin wallet flow before public packs are enabled.</p></div>
<div class="pack"><div class="row"><div><div class="coins">🪙 1 Fame Coin</div><div class="pill">OWNER QA ONLY</div></div><div class="price">$0.99</div></div></div>
<div id="success" class="success">Payment verified. Your Fame Coin wallet was credited. Return to Fameverse and refresh the gift tray.</div>
${action}
<div id="error" class="error"></div>
<p class="fine">The public Fame Coin package values are not locked by this QA purchase. Fameverse credits coins only after PayPal confirms a completed capture on the server.</p>
</main>
<script>
...
${configured ? `paypal.Buttons({
  style:{layout:'vertical',shape:'pill',label:'paypal'},
  createOrder:async()=>{errorEl.textContent='';const data=await post({action:'create',pack_id:'owner-qa-99c'});return data.order_id;},
  onApprove:async(data)=>{errorEl.textContent='';const result=await post({action:'capture',order_id:data.orderID});successEl.style.display='block';document.getElementById('paypal-button-container').style.display='none';return result;},
  onCancel:()=>{errorEl.textContent='Payment cancelled. No Fame Coins were credited.';},
  onError:()=>{errorEl.textContent='PayPal could not complete this checkout. No Fame Coins were credited.';}
...
Deno.serve(async (req: Request) => {
  ...
  if (req.method === "POST") {
    ...
    if (action === "create") {
      ...
      try {
        ...
        const paypalResponse = await fetch(`${paypalBase(paypalEnvironment)}/v2/checkout/orders`, {
          ...
          body: JSON.stringify({
            ...
            purchase_units: [{
              ...
              description: `${pack.label} — ${pack.coins} Fame Coin${pack.coins === 1 ? "" : "s"}`,
        ...
        if (!paypalResponse.ok || !paypalOrder.id) {
          await admin.from("coin_recharge_orders").update({ status: "failed", updated_at: new Date().toISOString() }).eq("id", recharge.id);
    ...
    if (action === "capture") {
      ...
      if (recharge.status === "completed") {
        const { data: wallet } = await admin.from("beta_coin_wallets").select("balance").eq("user_id", session.user_id).maybeSingle();
        return json(200, { status: "completed", wallet_balance: wallet?.balance ?? 0, already_completed: true });
      }
      ...
      try {
        ...
        if (!captureResponse.ok || captured.status !== "COMPLETED") return json(502, { error: "paypal-capture-not-completed" });
        ...
        await admin.from("coin_recharge_sessions").update({ revoked_at: new Date().toISOString() }).eq("id", session.id);
        return json(200, {
          status: "completed",
  ...
  const session = await sessionContext(admin, sessionToken);
  if (!session) return html(401, "<h1>Recharge session expired</h1><p>Return to Fameverse and open Recharge again.</p>");
  if (!(await requireOwner(admin, session.user_id))) return html(403, "<h1>Recharge unavailable</h1>");
  return html(200, page(paypalClientId, sessionToken, Boolean(paypalClientId && paypalSecret)));
});
