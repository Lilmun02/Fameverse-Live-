...
export default function App() {
  ...
  const shareRoom = async (roomOverride = null) => {
    ...
    const roomTitle = room?.title || 'Fameverse'
    const creatorName = room?.host?.displayName || room?.host?.username || displayName
    const shareData = {
      title: roomTitle,
      text: room ? `${creatorName} is live on Fameverse: ${roomTitle}` : `${creatorName} on Fameverse`,
    ...
    try {
      if (navigator.share) await navigator.share(shareData)
        ...
        setToast(room ? 'Live link copied' : 'Fameverse link copied')
      }
    } catch {
