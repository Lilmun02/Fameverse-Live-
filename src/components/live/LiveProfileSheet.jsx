...
export default function LiveProfileSheet({
  sheet,
  currentUserId,
  followNetwork,
}) {
  ...
  const toggleFollow = () => {
    if (isSelf || !sheet.userId) return
    void Promise.resolve(followNetwork?.toggleFollow?.(sheet.userId)).then(() => {
      sheet.open(sheet.userId)
    })
  }
