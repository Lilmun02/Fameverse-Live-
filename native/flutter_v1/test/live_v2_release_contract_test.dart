...
void main() {
  group('Build 16 Live V2 release contract', () {
    test('gift catalog is unique, priced, and cinematic assets are intentional', () {
      ...
      for (final gift in fvGiftCatalog) {
        expect(gift.cost, greaterThan(0), reason: '${gift.id} must have a positive cost.');
        expect(gift.label.trim(), isNotEmpty, reason: '${gift.id} needs a label.');
    ...
    test('host and viewer use V2 stage and stale gift overlays are keyed per playback', () async {
      ...
      expect(viewer, contains("ValueKey<String>('viewer-gift-\$_giftSerial')"));
    });

    test('custom send keeps real gift visual and 100000 quantity ceiling', () async {
      final components = await File(
        'lib/features/live/native_live_components.dart',
      ).readAsString();

      expect(components, contains('NativeGiftTrayVisual(gift: _selected, size: 50)'));
      expect(components, contains('value.clamp(1, 100000)'));
      expect(components, contains("Text('Send ×\$quantity')"));
    });

    test('viewer cannot publish media before cohost permission and realtime does not self-echo', () async {
      final viewer = await File(
        'lib/features/live/stream_viewer_live_screen.dart',
      ).readAsString();
      final host = await File(
      ...
      expect(viewer, contains('await call.setMicrophoneEnabled(enabled: true)'));
      expect(viewer, contains('await call.setCameraEnabled(enabled: true)'));
      expect(backend, contains('RealtimeChannelConfig(ack: false, self: false)'));
    });
  });
}
