...
void main() {
  group('next TestFlight candidate product contract', () {
    test(
      'existing users land on sign in and do not get forced into signup',
      () async {
        final auth = await File(
    ...
    test(
      'recharge exposes exactly three fixed reference packs plus custom',
      () async {
        ...
        expect(pricing, contains("('owner-qa-100', '100 Fame Coins', 100, 99"));
        expect(
          pricing,
        ...
        expect(pricing, contains("('owner-qa-custom', 'Custom Fame Coins'"));
