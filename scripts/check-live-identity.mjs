...
assert.match(app, /useGiftSystem\(\{[\s\S]*?displayName,[\s\S]*?actorId,[\s\S]*?setToast,/, 'Gift activity must receive the signed-in actor id even when additional real gift metadata is present.')
assert.match(app, /useLiveActivity\(\{[\s\S]*?roomId: activeActivityRoomId,[\s\S]*?displayName,[\s\S]*?actorId,/, 'Live activity must receive the signed-in actor id.')
...
assert.doesNotMatch(sheet, /Top Fan|rank:/, 'Do not invent Top Fan or arbitrary rank data.')
assert.match(profiles, /from\('gifter_stats'\)/, 'Any gift level displayed in the profile sheet must come from the real gifter stats table.')
assert.doesNotMatch(profiles, /gifterLevel:\s*[2-9]\d*|badge:\s*['"]|rank:\s*['"]/, 'Profile loader must not hard-code gift levels, badges, or ranks.')
