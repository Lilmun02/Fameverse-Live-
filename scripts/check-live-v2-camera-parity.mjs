...
if (failures.length) {
  console.error('Host Live V2 camera-slot parity guard failed:')
  for (const failure of failures) console.error(`- ${failure}`)
  process.exit(1)
}

console.log('Host Live V2 camera-slot parity guard passed: both camera facings stay on the same full-canvas compositor path.')
